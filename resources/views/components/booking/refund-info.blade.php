...
<section class="bg-white rounded-2xl border border-purple-200 shadow-sm p-6">
    ...
    <div class="bg-purple-50 rounded-xl p-4 border border-purple-200 space-y-3">
        <div class="grid grid-cols-2 gap-4">
            <div>
                <p class="text-xs font-semibold text-purple-600 uppercase tracking-wider mb-1">Status Refund</p>
            ...
            <div>
                <p class="text-xs font-semibold text-purple-600 uppercase tracking-wider mb-1">Refund Diproses Pada</p>
                <p class="text-sm text-purple-800">
                    @if($booking->refund_processed_at)
                    {{ \Carbon\Carbon::parse($booking->refund_processed_at)->locale('id')->translatedFormat('d M Y \p\u\k\u\l H:i') }}
                    @else
                    -
                    @endif
                </p>
