                    ...
                    <div class="flex items-start space-x-4">
                        <div class="flex-shrink-0">
                            @if($booking->counselor->user->profile_pic)
                                <img
                                    src="{{ asset('storage/' . $booking->counselor->user->profile_pic) }}"
                                    class="w-16 h-16 rounded-full object-cover border"
                                >
                            @else
                                <div class="w-16 h-16 rounded-full bg-primary flex items-center justify-center">
                                    <span class="text-xl font-bold text-white">
                                        {{ strtoupper(substr($booking->counselor->user->name, 0, 1)) }}
                                    </span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-3 space-y-2 text-sm text-gray-600">
                                <div class="flex items-center">
                                    ...
                                    {{ $booking->counselor->user->email }}
                                </div>

                                @if($booking->counselor->user->phone)
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                                    </svg>
                                    {{ $booking->counselor->user->phone }}
                                </div>
                                @endif

                                <div class="flex items-center">
                                    <span class="text-gray-500 mr-1">Harga / sesi:</span>
                                    <span class="font-semibold text-gray-900">
                                        Rp {{ number_format($booking->counselor->price_per_session, 0, ',', '.') }}
                                    </span>
                                </div>
                            </div>
                        </div>
