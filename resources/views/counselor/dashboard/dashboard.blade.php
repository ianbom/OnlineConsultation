            ...
            <div class="grid xl:grid-cols-3 gap-6 mb-6">
                <div class="panel h-full xl:col-span-2">
                    <div class="flex items-center dark:text-white-light mb-5">
                        <h5 class="font-semibold text-lg">Income Overview</h5>
                    </div>
                    <p class="text-lg dark:text-white-light/90">Total Income <span
                            class="text-primary ml-2">Rp {{ number_format($totalIncome, 0, ',', '.') }}</span></p>
                                ...
                                <tr class="text-white-dark hover:text-black dark:hover:text-white-light/90 group">
                                    ...
                                    <td>
                                        @if($booking->payment && $booking->payment->status == 'success')
                                            <span class="badge bg-success shadow-md dark:group-hover:bg-transparent">Paid</span>
                                        @elseif($booking->payment && $booking->payment->status == 'pending')
                                            <span class="badge bg-warning shadow-md dark:group-hover:bg-transparent">Pending</span>
                                        @else
                                            <span class="badge bg-danger shadow-md dark:group-hover:bg-transparent">Failed</span>
                                        @endif
                                    </td>
                                    <td>
        ...
        document.addEventListener("alpine:init", () => {
            Alpine.data("sales", () => ({
                init() {
                    ...
                    const chartIncomeData = @json($chartIncome);

                    setTimeout(() => {
                        this.revenueChart = new ApexCharts(this.$refs.revenueChart, this.revenueChartOptions(chartIncomeData))
                    ...
                    this.$watch('$store.app.theme', () => {
                        isDark = this.$store.app.theme === "dark" || this.$store.app.isDarkMode ? true : false;
                        this.revenueChart.updateOptions(this.revenueChartOptions(chartIncomeData));
                ...
                revenueChartOptions(incomeData) {
                    return {
                        series: [{
                            name: 'Income',
                            data: incomeData
