...
export default function BookingDetail({ booking }: Props) {
  ...
  const handlePrintBooking = async () => {
    ...
    doc.setFontSize(8);
    doc.setTextColor(255, 255, 255);
    doc.text(statusText.toUpperCase(), pageWidth / 2, 31, { align: 'center' });
    ...
    rightY = drawRightRow('Nama', booking.counselor.user.name, rightY, true);
    rightY = drawRightRow(
      'Spesialisasi',
    ...
    if (booking.payment) {
      ...
      y = drawRow('Total Bayar', formatCurrency(booking.payment.amount), y);
